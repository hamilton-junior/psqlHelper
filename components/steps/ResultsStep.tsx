...
interface ResultsStepProps {
  ...
  isTransactionActive?: boolean; // Prop adicionada para saber se há transação aberta
  onSaveEdits: (sql: string) => Promise<void>; // Função delegada para o App.tsx
}
    ...
    const fetchAllPreviews = async () => {
      ...
      links.forEach(async (link) => {
        try {
          ...
          if (isMounted) { if (results && results.rows.length > 0) { const val = results.rows[0][link.previewCol]; setPreviews(prev => ({ ...prev, [link.id]: { data: val === null ? 'NULL' : String(val), loading: false, error: false } })); } else { setPreviews(prev => ({ ...prev, [link.id]: { data: 'Não encontrado', loading: false, error: false } })); } }
...
const ResultsStep: React.FC<ResultsStepProps> = ({ data, sql, onBackToBuilder, onNewConnection, settings, onShowToast, credentials, executionDuration, schema, resultsState, onResultsStateChange, isTransactionActive, onSaveEdits }) => {
  ...
  const sqlStatementsPreview = useMemo(() => { 
     ...
     let lines = [];
     // Se não houver transação ativa, o script precisa ser atômico por si só.
     if (!isTransactionActive) lines.push("BEGIN; -- Início da transação de auditoria");
     ...
     if (!isTransactionActive) lines.push("COMMIT; -- Persistência definitiva"); 
     else lines.push("-- Alterações enviadas para a transação aberta.");

     return lines.join('\n'); 
  }, [pendingEdits, localData, mainTableName, finalPkColumn, isTransactionActive]);
  ...
  const handleSaveChanges = async () => { 
    if (!sqlStatementsPreview) return; 
    setIsSaving(true); 
    try { 
      await onSaveEdits(sqlStatementsPreview); 
      onShowToast(isTransactionActive ? "Comandos adicionados à transação ativa." : "Alterações salvas com sucesso.", "success"); 
      setPendingEdits({}); 
    } catch (e: any) { 
