...
import { Server, Shield, Info, Loader2, Database, AlertCircle, Bot, Wand2 } from 'lucide-react';
    ...
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
          ...
          {mode === 'real' ? 'Conexão ao Banco de Dados' : 'Simulação com IA'}
        </h2>
        <p className="text-slate-500 dark:text-slate-400 mt-2">
          {mode === 'real' 
            ? 'Conecte-se a instância de seu Banco de Dados PostgreSQL local.' 
            : 'Gere um banco de dados virtual instantaneamente para testar queries.'}
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        ...
        <div className="flex border-b border-slate-200 dark:border-slate-700">
          ...
          >
             <div className="flex items-center justify-center gap-2">
                <Bot className="w-4 h-4" /> Modo Simulação (IA)
             </div>
          </button>
        </div>
          ...
          {mode === 'real' ? (
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-300">
              <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-xl flex gap-3 text-indigo-900 dark:text-indigo-200 text-sm">
                ...
                <p>
                  Não tem um banco de dados rodando? Sem problemas! 
                  A IA pode criar uma <strong>estrutura simulada</strong> baseada na sua ideia.
                  Você poderá construir queries SQL reais contra essa estrutura.
                </p>
              </div>
              ...
              >
                {loading ? (
                  <>
                    ...
                    {mode === 'real' ? 'Conectando...' : 'Gerando Schema...'}
                  </>
                ) : (
